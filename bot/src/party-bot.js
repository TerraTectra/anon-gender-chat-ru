...
export function createPartyBot(token, dbPath, random = Math.random) {
  ...
  bot.callbackQuery(/^party:pick:(would|truth|dare|mime|icebreaker)$/, async (ctx) => {
    const category = ctx.match[1];
    store.recordAction(ctx.from.id, category);
    await ctx.answerCallbackQuery();
    await ctx.editMessageText(`${LABELS[category]}\n\n${pick(category)}`, {
      reply_markup: new InlineKeyboard().text("Ещё", `party:pick:${category}`).text("Режимы", "party:menu")
    });
  });

  bot.catch((error) => console.error("Party bot error", error.error));
  return bot;
}
