...
export function createDatingBot(token, dbPath, random = Math.random) {
  ...
  return attachBasics(bot,store,{name:"Tectra Meet 18+",short:"Знакомства по городу и интересам, без свайп-бесконечности.",description:"Создайте короткую анкету, находите людей рядом по городу и интересам и открывайте контакт только при взаимной симпатии. Только 18+.",commands:[{command:"profile",description:"создать анкету 18+"},{command:"find",description:"найти анкету"},{command:"like",description:"симпатия по ID"},{command:"pause",description:"скрыть анкету"}]});
}
