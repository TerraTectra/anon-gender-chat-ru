...
CREATE INDEX IF NOT EXISTS idx_language_events ON events(type, created_at);
`;
...
export class LanguageStore {
  ...
  setProfile(id, patch) {
    const allowed = new Set(["age_group", "level", "state"]);
  ...
  enqueue(userId) {
    ...
    if (!user?.age_group || !user?.level) return { status: "profile_required" };
    this.disconnect(userId);
    this.db.prepare("INSERT OR REPLACE INTO queue (user_id, created_at) VALUES (?, CURRENT_TIMESTAMP)")
      .run(userId);
  ...
  tryMatch(userId) {
    ...
    `).all(userId, user.age_group, userId, userId);
    const match = candidates.find((candidate) => Math.abs(LEVEL_RANK[user.level] - LEVEL_RANK[candidate.level]) <= 1);
